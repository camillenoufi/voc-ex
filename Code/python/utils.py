...
def pad_sents_char(sents, char_pad_token):
    ...
    for s in sents:
        ...
        for w in s:
            ...
            padded_w[:len(w[:max_word_length])] = w[:max_word_length]
            if w[-1] not in padded_w: 
                padded_w[-1] = w[-1]  
            padded_sent.append(padded_w)
        
        padding = max_sent_len - len(padded_sent)
        pad_word = [char_pad_token] * max_word_length
        extras = [pad_word] * padding
        padded_sent.extend(extras)
